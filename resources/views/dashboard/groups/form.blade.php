...
@section('tab-parent')
<a class="active" href="{{ route('group.index') }}" title="">Lista de Grupos</a>
...
<div class="body-content">
    <form action="{{ $data['action'] }}" class="form">
        ...
        <input type="text" name="name" id="name" value="{{$data['array']  ? $data['array']->name : ''}}" {{ $data['view'] ? 'disabled' : '' }}>
		</div>
        <div class="data-option">
		<label for="fee">Cuota</label>
        <input type="text" class="numeric" name="fee" id="fee" value="{{$data['array']  ? $data['array']->fee : 20 }}" {{ $data['view'] ? 'disabled' : '' }}>
		</div>
        <div class="data-option">
		<label for="branch">Total de Registros por Patrocinador</label>
        <input type="text" class="integer" name="branch" id="branch" value="{{$data['array']  ? $data['array']->branch : 2 }}" {{ $data['view'] ? 'disabled' : '' }}>
		</div>
